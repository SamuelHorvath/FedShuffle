...
class ShakespeareFL(Dataset):
    ...
    """
    # TODO: needs to be revisited, especially test loader,
    #  necessary for personalized FL
...
#     #  necessary for personalized FL
#     def __init__(self, data_path, train=True,
#                  batch_size=SHAKESPEARE_EVAL_BATCH_SIZE, client_id=None):
#         self.train = train
#         if train:
#             data_path = os.path.join(
#                 data_path, 'shakespeare/shakespeare_train.h5')
...
#         # append zeros if necessary
#         x_data = torch.cat([x_data, torch.zeros(
#             n_zeros, self.dataset.seq_len).long()], dim=0)
#         y_data = torch.cat([y_data, torch.zeros(
#             n_zeros, self.dataset.seq_len).long()], dim=0)
...
#         super(ShakespeareH5, self).__init__(
#             h5_path, cliend_id, 'shakespeare', 'snippets')
#         self.seq_len = seq_len
#         # vocabulary
#         self.vocab = SHAKESPEARE_VOCAB
#         self.char2idx = {u: i for i, u in enumerate(self.vocab, 1)}
...
#         indices = np.array([self.char2idx[char] if char in self.char2idx else
#                            self.oov for char in record])
#         len_chars = 1 + len(indices)  # beginning of speech
#         pad_size = int(np.ceil(
#             len_chars/self.seq_len) * self.seq_len - len_chars)
#         indices = np.concatenate(
#             ([self.bos], indices, [self.eos], torch.zeros(pad_size)), axis=0)
